...
import './DataManagerB.sol';

contract DFSTokenBControl is ERC20, DataManagerB {
  ...
  function _transfer(address _from, address _to, uint256 _amount, bytes _data) internal {
    ...
    require(_amount > 0);
    require(_from != _to);
    _setDFBBalanceOf(_from, balanceOf(_from).sub(_amount));
    _setDFBBalanceOf(_to, balanceOf(_to).add(_amount));
  ...
  function mint(address _to, uint256 _amount) whenNotPaused onlyOwner canMint public returns (bool) {
    bytes memory empty;
    _setTotalSupplyDFB(totalSupply().add(_amount));
    _setDFBBalanceOf(_to, balanceOf(_to).add(_amount));
